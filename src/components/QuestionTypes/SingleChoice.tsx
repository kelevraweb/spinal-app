...
const SingleChoice: React.FC<SingleChoiceProps> = ({
  ...
  const iconMap: Record<string, any> = {
    ...
    'spine': faSpinner,
    'chart': faChartLine,
    'young': faUserNinja,    // Icon for young age group
    'adult': faPersonWalking, // Icon for adult age group
    'middle': faRunning,     // Icon for middle-aged
  ...
  if (useImages && questionId === 'gender') {
    return <div className="grid grid-cols-2 gap-4 mt-6">
        <button type="button" className={`flex flex-col items-center p-4 rounded-lg border-2 transition-all ${value === 'Maschio' ? 'border-[#71b8bc] bg-[#71b8bc]/10' : 'border-gray-300 hover:border-gray-400'}`} onClick={() => handleSelection('Maschio')}>
        ...
        <button type="button" className={`flex flex-col items-center p-4 rounded-lg border-2 transition-all ${value === 'Femmina' ? 'border-[#71b8bc] bg-[#71b8bc]/10' : 'border-gray-300 hover:border-gray-400'}`} onClick={() => handleSelection('Femmina')}>
          <div className="w-32 h-32 rounded-full overflow-hidden mb-3">
  ...
  if (questionId === 'daily_activity') {
    return <div className="flex flex-col space-y-4 mt-6">
        <button type="button" className={`flex items-center justify-between p-4 rounded-lg border-2 transition-all ${value === 'Quasi nulla' ? 'border-[#71b8bc] bg-[#71b8bc]/10' : 'border-gray-300 hover:border-gray-400'}`} onClick={() => handleSelection('Quasi nulla')}>
        ...
        <button type="button" className={`flex items-center justify-between p-4 rounded-lg border-2 transition-all ${value === 'Solo camminate leggere' ? 'border-[#71b8bc] bg-[#71b8bc]/10' : 'border-gray-300 hover:border-gray-400'}`} onClick={() => handleSelection('Solo camminate leggere')}>
        ...
        <button type="button" className={`flex items-center justify-between p-4 rounded-lg border-2 transition-all ${value === 'Faccio sport o esercizi regolari' ? 'border-[#71b8bc] bg-[#71b8bc]/10' : 'border-gray-300 hover:border-gray-400'}`} onClick={() => handleSelection('Faccio sport o esercizi regolari')}>
        ...
        <button type="button" className={`flex items-center justify-between p-4 rounded-lg border-2 transition-all ${value === 'Alterno periodi attivi e sedentari' ? 'border-[#71b8bc] bg-[#71b8bc]/10' : 'border-gray-300 hover:border-gray-400'}`} onClick={() => handleSelection('Alterno periodi attivi e sedentari')}>
      ...
      return <button key={index} type="button" className={`option-btn ${value === optionText ? 'selected' : ''}`} onClick={() => handleSelection(optionText)}>
            <div className="flex items-center w-full">
              <div className={`icon-circle ${value === optionText ? 'bg-[#88c2aa]/30' : 'bg-gray-100'}`}>
                <FontAwesomeIcon icon={icon} className={value === optionText ? 'text-[#71b8bc]' : 'text-gray-500'} size="lg" />
              </div>
